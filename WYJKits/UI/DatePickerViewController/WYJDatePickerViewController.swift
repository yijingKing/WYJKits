...
///时间选择器
open class WYJDatePickerViewController: UIViewController {
    
    private var type: WYJComponentsType?
    ...
    public var containV: UIView = {
        let view = UIView(frame: CGRect(x: 0, y: UIScreen.main.bounds.height - WYJRatio(260), width: UIScreen.main.bounds.width, height: WYJRatio(260)))
    ...
    private func drawMyView() {
        ...
        cancelBtn = UIButton(frame: CGRect(x: 0, y: 0, width: WYJRatio(80), height: WYJRatio(40)))
        sureBtn = UIButton(frame: CGRect(x: UIScreen.main.bounds.width - WYJRatio(80), y: 0, width: WYJRatio(80), height: WYJRatio(40)))
        ...
        picker = UIPickerView(frame: CGRect(x: 0, y: WYJRatio(40), width: UIScreen.main.bounds.width, height: WYJRatio(220)))
...
// MARK: - 转场动画delegate
extension WYJDatePickerViewController: UIViewControllerTransitioningDelegate {
    public func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        let animated = WYJDatePickerViewControllerAnimated(type: .present)
        return animated
    }
    public func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        let animated = WYJDatePickerViewControllerAnimated(type: .dismiss)
        return animated
    }
}
//DatePickerViewController的推出和取消动画
private class WYJDatePickerViewControllerAnimated: NSObject,UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        switch type {
        case .present:
            guard let toVC = transitionContext.viewController(forKey: .to) as? WYJDatePickerViewController else {
                return
            }
        ...
        case .dismiss:
            guard let toVC = transitionContext.viewController(forKey: .from) as? WYJDatePickerViewController else {
                return
            }
            UIView.animate(withDuration: 0.25, animations: {
...
//MARK: --- 调用方法
extension WYJDatePickerViewController {
    /// 年月日
    /// - Parameters:
    ///   - types: 类型
    ///   - startTimes: 开始时间
    ...
    ///   - sureBtnFont: 字体
    ///   - block: 确认回调
    public class func show(_ type: WYJComponentsType,
                              _ startTime: Int? = nil,
                              _ cancel: String? = nil,
        ...
        let vc = WYJDatePickerViewController()
        vc.showDatePicker(type, startTime, cancel, cancelBtnColor, cancelBtnFont, sure, sureBtnColor, sureBtnFont, block)
    ...
    public func showDatePicker(_ types: WYJComponentsType,
        ...
        cancelBtn.setTitleColor(cancelBtnColor ?? WYJ99Color, for: .normal)
        cancelBtn.titleLabel?.font = UIFont.systemFont(ofSize: cancelBtnFont ?? 15)
        sureBtn.setTitle(sure ?? "确定", for: .normal)
        sureBtn.setTitleColor(sureBtnColor ?? WYJHexColor("#1C81FE"), for: .normal)
