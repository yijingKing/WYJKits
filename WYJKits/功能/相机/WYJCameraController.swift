...
///拍照
open class WYJCameraController: WYJBaseViewController {
    public var photoType: WYJPhotoType?
    public var cameraDidFinishShoot: ((UIImage)->())?
    ...
    fileprivate lazy var floatingView: WYJFloatingView = {
        let v = WYJFloatingView(frame: view.bounds)
        v.frame = view.bounds
    ...
    private lazy var photoButton: UIButton = {
        return UIButton().yi.then({
            $0.yi.backgroundImage(WYJFloatingView.bundledImage(named: "photo"))
    ...
    private lazy var cancleButton: UIButton = {
        return UIButton().yi.then({
            ...
            $0.yi.backgroundImage(WYJFloatingView.bundledImage(named: "closeButton"))
            $0.addTarget(self, action: #selector(cancleButtonAction(_ :)), for: .touchUpInside)
        })
    }()
    private lazy var flashButton: UIButton = {
        return UIButton().yi.then({
            $0.yi.backgroundImage(WYJFloatingView.bundledImage(named: "cameraFlash"))
    ...
    private func interfaceLayout() {
        view.yi.addSubviews([photoButton,cancleButton,flashButton,bottomView,titleLabel])
        photoButton.snp.makeConstraints({
            $0.centerX.equalToSuperview()
            $0.bottom.equalTo(-WYJBottomHeight - WYJRatio(27))
        })
        cancleButton.snp.makeConstraints({
            $0.centerY.equalTo(photoButton.snp.centerY)
            $0.left.equalTo(photoButton.snp.right).offset(WYJRatio(77))
        })
        flashButton.snp.makeConstraints({
            $0.right.equalTo(photoButton.snp.left).offset(WYJRatio(-77))
            $0.centerY.equalTo(photoButton.snp.centerY)
        })
        bottomView.snp.makeConstraints({
            $0.left.right.equalToSuperview()
            $0.bottom.equalTo(-WYJBottomHeight)
            $0.height.equalTo(WYJRatio(55))
        })
        
        titleLabel.snp.makeConstraints({
            $0.centerX.equalTo(WYJRatio(27))
...
extension WYJCameraController: AVCapturePhotoCaptureDelegate {
    ///获取到照片
...
fileprivate class WYJFloatingView: UIView {
    var photoType: WYJPhotoType? {
        willSet {
            switch newValue {
                case .normal:
    ...
    lazy var frontImage: UIImageView = {
        return UIImageView().yi.then({
            $0.image = WYJFloatingView.bundledImage(named: "xuxian")
            $0.transform = CGAffineTransform.init(rotationAngle: CGFloat.pi * 0.5)
    ...
    lazy var reverseImage: UIImageView = {
        return UIImageView().yi.then({
            $0.image = WYJFloatingView.bundledImage(named: "Page")
            $0.transform = CGAffineTransform.init(rotationAngle: CGFloat.pi * 0.5)
            $0.contentMode = .scaleAspectFill
        })
    }()
    ...
    internal class func bundledImage(named name: String) -> UIImage {
        let primaryBundle = Bundle(for: WYJCameraController.self)
        if let image = UIImage(named: name, in: primaryBundle, compatibleWith: nil) {
            return image
        } else if
            let subBundleUrl = primaryBundle.url(forResource: "WYJResources", withExtension: "bundle"),
    ...
    override init(frame: CGRect) {
        ...
        let width: CGFloat = WYJRatio(270)
        ...
        frontImage.isHidden = true
        reverseImage.isHidden = true
        yi.addSubviews([frontImage,reverseImage])
        reverseImage.snp.makeConstraints({
            $0.centerY.equalToSuperview().offset(-height / 2 + WYJRatio(72))
            $0.centerX.equalToSuperview().offset(width / 4)
            $0.width.height.equalTo(WYJRatio(92))
        })
        frontImage.snp.makeConstraints({
            $0.centerY.equalToSuperview().offset(height / 2 - WYJRatio(92))
            $0.centerX.equalToSuperview().offset(WYJRatio(20))
            $0.width.equalTo(WYJRatio(145))
            $0.height.equalTo(WYJRatio(130))
        })
    }
...
extension UIImage {
    ...
    public func dx_clipImageInRect(rect: CGRect) -> UIImage?{
        ...
        let widthScale: CGFloat = size.width / WYJScreenWidth
        let heightScale: CGFloat = size.height / WYJScreenHeight
        
        //其实是横屏的
        let originWidth: CGFloat = rect.size.width
        let originHeight: CGFloat = rect.size.height
        
        let x: CGFloat = (WYJScreenHeight - originHeight) * 0.5 * heightScale
        let y: CGFloat = (WYJScreenWidth - originWidth) * 0.5 * widthScale
