...
///正则表达式
open class WYJRegular: NSObject {
    public enum WYJValidatedType {
        case Email
        case PhoneNumber
        case Number
    ...
    ///验证邮箱
    public class func isEmail(_ vStr: String) -> Bool {
        return validateText(.Email, vStr)
    }

    ///验证手机号
    public class func isPhoneNumber(_ vStr: String) -> Bool {
        return validateText(.PhoneNumber, vStr)
    }

    ///验证纯数字
    public class func isNumber(_ vStr: String) -> Bool {
        return validateText(.Number, vStr)
    }

    ///验证数字个数
    public class func isSpecificNumbers(_ vStr: String,_ num: Int) -> Bool {
        return validateText(.SpecificNumbers, vStr,num)
    }

    ///验证是否是中文
    public class func isChinese(_ vStr: String) -> Bool {
        return validateText(.Chinese, vStr)
    }

    ///验证是否含有^%&',;=?$\"等字符
    public class func isIllegalCharacter(_ vStr: String) -> Bool {
        return validateText(.IllegalCharacter, vStr)
    }

    ///验证URL
    public class func isURL(_ vStr: String) -> Bool {
        return validateText(.URL, vStr)
    }

    ///验证首尾空白行
    public class func isBlankLines(_ vStr: String) -> Bool {
        return validateText(.BlankLines, vStr)
    }

    ///验证QQ号
    public class func isQQ(_ vStr: String) -> Bool {
        return validateText(.QQ, vStr)
    }

    ///验证ID地址
    public class func isID(_ vStr: String) -> Bool {
        return validateText(.ID, vStr)
    }

    ///验证MAC地址
    public class func isMAC(_ vStr: String) -> Bool {
        return validateText(.MAC, vStr)
    }

    ///验证身份证号
    public class func isIdCard(_ vStr: String) -> Bool {
        return validateText(.IdCard, vStr)
    }

    ///验证年月日    例子 2013-04-1 2
    public class func isDateInformation(_ vStr: String) -> Bool {
        return validateText(.DateInformation, vStr)
    }

    ///验证帐号是否合法(字母开头，默认允许6-18字节，允许字母数字下划线)
    public class func isAccountLegal(_ vStr: String,
                                 _ num: Int? = nil,
                                 _ num2: Int? = nil) -> Bool {
        return validateText(.AccountLegal, vStr,num,num2)
    }
    ///验证密码(以字母开头，默认长度在6~18之间，只能包含字母、数字和下划线)
    public class func isPasswordIsValidated(_ vStr: String,
                                        _ num: Int? = nil,
                                        _ num2: Int? = nil) -> Bool {
        return validateText(.Password, vStr)
    }
    ///验证强密码(必须包含大小写字母和数字的组合，不能使用特殊字符，长度在8-18之间)
    public class func isStrongPassword(_ vStr: String,
                                   _ num: Int? = nil,
                                   _ num2: Int? = nil) -> Bool {
        return validateText(.StrongPassword, vStr)
    }
    ///验证
    private class func validateText(_ type: WYJValidatedType,
