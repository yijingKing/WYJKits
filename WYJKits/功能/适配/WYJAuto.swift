...
public enum WYJAuto {
    public static func set(_ base: Double) {
        WYJAuto.set { origin in
...
extension WYJAuto {
    ...
    private static func set(conversion: @escaping ((Double) -> Double)) {
        conversionClosure = conversion
    }
    
    /// 转换 用于数值的等比例计算 如需自定义可重新设置
    private static var conversionClosure: ((Double) -> Double) = { (origin) in
        guard UIDevice.current.userInterfaceIdiom == .phone else {
            return origin
        }
        let base = 375.0
...
public protocol WYJAutoCalculationable {
...
extension Double: WYJAutoCalculationable {
    
    public func auto() -> Double {
        return WYJAuto.conversion(self)
    }
}
...
extension CGPoint: WYJAutoCalculationable {
...
extension CGSize: WYJAutoCalculationable {
...
extension CGRect: WYJAutoCalculationable {
...
extension CGVector: WYJAutoCalculationable {
...
extension UIOffset: WYJAutoCalculationable {
...
extension UIEdgeInsets: WYJAutoCalculationable {
